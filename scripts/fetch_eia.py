"""Optional reproducibility snapshot utility.

The GridPulse dashboard queries EIA directly. This script exists only when a dated,
versionable local snapshot is useful for a model experiment or audit trail.
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create an optional EIA-930 reproducibility snapshot.")
    ...
    parser.add_argument("--output", type=Path, default=Path("data/processed/hourly_grid_snapshot.csv"))
    ...
    print(f"Saved optional snapshot with {len(enriched):,} hourly rows to {args.output}")
